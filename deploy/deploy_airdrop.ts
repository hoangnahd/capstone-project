...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Load merkle root
  const rootPath = './output/merkleRoot.txt';
  const merkleRoot = fs.readFileSync(rootPath, 'utf8').trim();

  // Deploy AirDrop as an upgradeable contract
  const airDrop = await deploy("MerkleAirdropUpgradeable", {
    contract: "MerkleAirdropUpgradeable",
    from: deployer,
    log: true,
    proxy: {
      proxyContract: "UUPS",
